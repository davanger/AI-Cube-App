async function connect()
{
    try
    {
        ...
        device = await window.navigator.bluetooth.requestDevice({ filters: [{ namePrefix: "GoCube_" }], optionalServices: [GOCUBE_SERVICE_UUID] });
        ...
        var service = await server.getPrimaryService(GOCUBE_SERVICE_UUID);
        console.log("Service: ", service);

        var characteristics = await service.getCharacteristics(GOCUBE_CHARACTERISTIC_UUID);
