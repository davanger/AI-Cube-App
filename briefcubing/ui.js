var Ui = (function () {
    ...
    function validate_twist(user_t, solution_t) {
        if(user_t == solution_t) {
            return true;
        }
    }
    ...
    function update_step_button() {
        switch (window.current_step){
            case "Cross":
                var cross_button = document.getElementById("cross_button");
                var f2l_button = document.getElementById("f2l_button");
                cross_button.style.color = "green";
                f2l_button.disabled = false;
                break;
            case "F2L":
                var f2l_button = document.getElementById("f2l_button");
                var oll_button = document.getElementById("oll_button");
                f2l_button.style.color = "green";
                oll_button.disabled = false;
                break;
            case "OLL":
                var oll_button = document.getElementById("oll_button");
                var pll_button = document.getElementById("pll_button");
                oll_button.style.color = "green";
                pll_button.disabled = false;
                break;
            case "PLL":
                var pll_button = document.getElementById("pll_button");
                pll_button.style.color = "green";
                break;
            default:
                break;
        }
        if(window.current_step == "Cross"){
            
        }

    }
    function twist(t) {
        next_step = window.next_steps[window.next_steps.length - 1];
        if (validate_twist(t, next_step)) {
            window.scramble.push(next_step);
            window.next_steps.pop();
            if(window.correction_steps == 0) {
                moveForward();
            }
            else {
                window.correction_steps--;
            }
            var display_steps = [...next_steps];
            ...
            var correction_move = null;
            if(t.includes("'")) {
                correction_move = t.substring(0,1);
            }
            else {
                correction_move = t + "'";
            }
            window.next_steps.push(correction_move);
            window.correction_steps++;
...
// F_U2_L2_B2_FP_U_L2_U_R2_D2_LP_B_L2_BP_R2_U2
// F_U_U_L_L_B_B_FP_U_L_L_U_R_R_D_D_LP_B_L_L_BP_R_R_U_U